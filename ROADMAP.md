...
| A7 ☐ | **Profile pictures** — a small photo (or chosen icon/colour) per person, shown beside the name everywhere people appear: member chips, attribution lines ("Ana ticked…"), assignee/scope pickers, handoff lines, the circle roster. Also the care recipient / participant via their profile (C2a / About Me) — the subject of a space deserves a face, not just a header | **Accessibility is the point**: faces identify carer / patient / owner / child faster than text for non-readers, kids, people with cognitive disability, and aging carers. Rules: the photo NEVER replaces the name (photo + name always together; alt text = the name); an icon/colour fallback so nobody is a grey blank. Storage rides the C1d receipts decision — client-side crop to a tiny square (≤128px, ~5–15 KB) in Firestore, no Blaze/credit card needed; photo lives on the member's own membership entry / profile doc. Consent matters: photos of children and NDIS participants are sensitive — per-space opt-in, members-only visibility, participant/owner controls their own image (for Cooee this lands with the Phase 4 consent & visibility screen) |
...
| C2e ✅ v30 | **Shift handoff** — "since you last looked" divider in each space's history | Extends the existing family-activity banner + history machinery |
...
| P2 ✅ v35 | **AI proxy** — Cloud Function (`functions/`) holding the operator's Gemini key: verifies Auth token, per-uid daily quota (the free/paid dial), forwards summarize / break-down / tone / briefing. Client routes proxy-vs-key transparently; a pasted key always overrides | Code shipped + tested; **inert until the operator deploys it** (functions/README.md) |
